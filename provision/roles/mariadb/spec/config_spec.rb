...
describe 'role mariadb' do
  ...
  describe file('/etc/my.cnf.d/mysqld.cnf') do
    ...
    sql_mode_value = property['mariadb_cfg']['mysqld']['sql_mode']
    if property['mariadb_cfg']['mysqld']['sql_mode'].is_a?(Array)
      sql_mode_value = property['mariadb_cfg']['mysqld']['sql_mode'].join(',')
    end
    ...
    if property['mariadb_cfg']['mysqld'].has_key?('max_connections')
      its(:content) { should match /^max_connections = #{e(property['mariadb_cfg']['mysqld']['max_connections'])}$/ }
    end
    if property['mariadb_cfg']['mysqld'].has_key?('connect_timeout')
      its(:content) { should match /^connect_timeout = #{e(property['mariadb_cfg']['mysqld']['connect_timeout'])}$/ }
    end
    ...
    if property['mariadb_cfg']['mysqld'].has_key?('skip_name_resolve') and property['mariadb_cfg']['mysqld']['skip_name_resolve']
      its(:content) { should match /^skip_name_resolve$/ }
    end
    if property['mariadb_cfg']['mysqld'].has_key?('interactive_timeout')
      its(:content) { should match /^interactive_timeout = #{e(property['mariadb_cfg']['mysqld']['interactive_timeout'])}$/ }
    end
    if property['mariadb_cfg']['mysqld'].has_key?('wait_timeout')
      its(:content) { should match /^wait_timeout = #{e(property['mariadb_cfg']['mysqld']['wait_timeout'])}$/ }
    end
    if property['mariadb_cfg']['mysqld'].has_key?('table_open_cache')
      its(:content) { should match /^table_open_cache = #{e(property['mariadb_cfg']['mysqld']['table_open_cache'])}$/ }
    end
    ...
    if property['mariadb_cfg']['mysqld'].has_key?('slow_query_log')
      slow_query_log_value = property['mariadb_cfg']['mysqld']['slow_query_log'] ? '1' : '0'
      if property['mariadb_cfg']['mysqld']['slow_query_log'].is_a?(Integer) && property['mariadb_cfg']['mysqld']['slow_query_log'] == 0
    ...
    if property['mariadb_cfg']['mysqld'].has_key?('log_warnings')
      its(:content) { should match /^log_warnings = #{property['mariadb_cfg']['mysqld']['log_warnings']}$/ }
    end
    if property['mariadb_cfg']['mysqld'].has_key?('long_query_time')
      its(:content) { should match /^long_query_time = #{property['mariadb_cfg']['mysqld']['long_query_time']}$/ }
    end
    if property['mariadb_cfg']['mysqld'].has_key?('slow_query_log_file')
      its(:content) { should match /^slow_query_log_file = #{property['mariadb_cfg']['mysqld']['slow_query_log_file']}$/ }
    end
    if property['mariadb_cfg']['mysqld'].has_key?('general_log')
      slow_query_log_value = property['mariadb_cfg']['mysqld']['general_log'] ? '1' : '0'
      if property['mariadb_cfg']['mysqld']['general_log'].is_a?(Integer) && property['mariadb_cfg']['mysqld']['general_log'] == 0
        slow_query_log_value = '0'
      end
      its(:content) { should match /^general_log = #{property['mariadb_cfg']['mysqld']['general_log']}$/ }
    end
    if property['mariadb_cfg']['mysqld'].has_key?('general_log_file')
      its(:content) { should match /^general_log_file = #{property['mariadb_cfg']['mysqld']['general_log_file']}$/ }
    end
    if property['mariadb_cfg']['mysqld'].has_key?('gtid_domain_id')
      its(:content) { should match /^gtid_domain_id = #{property['mariadb_cfg']['mysqld']['gtid_domain_id']}$/ }
    end
    if property['mariadb_cfg']['mysqld'].has_key?('server_id')
      its(:content) { should match /^server_id = #{property['mariadb_cfg']['mysqld']['server_id']}$/ }
    end
    if property['mariadb_cfg']['mysqld'].has_key?('log_bin')
      its(:content) { should match /^log_bin = #{property['mariadb_cfg']['mysqld']['log_bin']}$/ }
    end
    if property['mariadb_cfg']['mysqld'].has_key?('binlog_format')
      its(:content) { should match /^binlog_format = #{property['mariadb_cfg']['mysqld']['binlog_format']}$/ }
    end
    if property['mariadb_cfg']['mysqld'].has_key?('expire_logs_days')
      its(:content) { should match /^expire_logs_days = #{property['mariadb_cfg']['mysqld']['expire_logs_days']}$/ }
    end
    if property['mariadb_cfg']['mysqld'].has_key?('read_only') && property['mariadb_cfg']['mysqld']['read_only']
      its(:content) { should match /^read_only$/ }
    end
    if property['mariadb_cfg']['mysqld'].has_key?('log_slave_updates') && property['mariadb_cfg']['mysqld']['log_slave_updates']
