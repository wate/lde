...
- 収束フェーズと判断される場合は、原則として新たな代替案を提示せず、指示内容の修正範囲内で対応する。
    - ただし、設計上重大な問題や明確なリスクがある場合に限り、その旨を明示した上で代替案を提示すること    
...
- 当初の相談目的や検討範囲から逸脱・拡張が生じる可能性がある場合は、提案を続ける前に方向性の確認を行い、確認が取れるまで新たな代替案は提示しないこと
...
**重要**: 作業開始前に該当するプロセス文書を必ず確認し、そこで定義された段階的情報取得手順に従うこと。
...
**注意**: 具体的な対象ファイルとインストラクションファイルの対応関係は、各プロセス文書で定義されています。
