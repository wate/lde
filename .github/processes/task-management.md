...
作業を効率的に進めるためのプロセスです。
作業指示書ファイル(*.md)が提供された場合、およびチャットで複数の作業指示が
一度に来た場合の標準手順を定義します。
...
複数の作業指示が一度に来た場合、以下の原則を必ず守ること。

- 作業プラン作成後、1件ずつ順番に実施する
- 1件完了ごとにユーザーに確認を求め、承認を得てから次の作業へ進む
- 承認前に次の作業を開始しない
...
- サブディレクトリ配下への作成は避ける
- 実施ログ(作業ログ)ファイルも同様にプロジェクトルートに作成する

#### 既存作業プランファイルの確認

- 決定したファイル名で既存の作業プランファイルを読み取り
...
- [AIペア作業フロー シーケンス図](./ai_pair_sequence.d2): タスク管理を含むAIペア作業の一連のフロー
