...
以下のいずれかに該当する場合、作業開始前に作業プランファイルを作成する。

- 複数日にわたる作業や粒度の大きいタスク
- チャットで複数の修正指示が一度に来た場合(作業指示書の有無を問わない)

1. 作業指示書の確認: チケット番号、要件、制約条件を把握
2. 作業プランファイルの作成: タスク分解、技術調査項目、完了条件を整理
3. 過去の指摘事項の確認: 類似作業での指摘を確認し、同様の問題を回避

詳細は[タスク管理プロセス](./task-management.md)を参照してください。
...
問題を発見したら、以下の基準で対応してください。この判定基準は[開発作業プロセスのボーイスカウトルール](./development.md)の「即座に修正可能か」判定に使用されます。
