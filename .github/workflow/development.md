...
- **コミット禁止**: `git commit`は実行しない。変更作業完了後はステージングまで実行し、`git status`で変更内容を表示してユーザーの確認を待つ(ユーザーが明示的にコミットを許可した場合を除く)
- **既存ファイルの更新**: `git add -u --sparse` で追跡済みファイルの変更を一括ステージング(誤追加防止)
- **新規ファイルの追加**: `git status` で未追跡ファイルを確認後、`git add --sparse <file>` で個別に明示的に追加
- **変更内容の確認**: `git diff --cached` でステージング内容、`git diff` で未ステージング変更を確認
